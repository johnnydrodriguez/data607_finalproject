...
For our final project, the team will analyze fuel consumption differences between electric (EV), gasoline and hybrid vehicles.  Specifically, we will address the question - **What are the cost differences - and savings - for each type of vehicle?**  Finally, we will (attempt to) implement a shiny app that allows users to select two vehicles to compare fuel consumption and cost differences between the selected vehicles.

The primary dataset for vehicle fuel consumption will be sourced from https://www.fueleconomy.gov/feg/download.shtml  - specifically, the 2022 vehicle fuel data:
https://www.fueleconomy.gov/feg/epadata/22data.zip

Fuel cost data will be sourced from https://www.eia.gov/petroleum/gasdiesel.  The weekly price data is located here:
...
> Working with a complex spreadsheet that needed substanstial cleanup
...
All vehicles are given a fuel efficiency score by the EPA.  What variables contribute to a fuel efficient vehicle?


* EPA Fuel Efficiency Score - which rates FE on a scale of 1-10 for all vehicles.
* Performed a multuple linear regression to identify which variable were highly correlated and predict the FE rating.
* We only looked at vehicles that the consumed gas to keep the analysis simple 


### Multiple Linear Regression Approach
Selected 11 variables from the data set

1. Checked for a linear relationships for each variable
2. Checked for residual distribution for each variable
3. Checked for collinearity
4. Isolated the variables that contribute to the Fuel Efficiency Rating

### Subset the data to create a model data set.

The predictor variables we picked are:
* engine displacement
* number of cylinders
* transmission type
* combined mpg
...
* FE ratings are nearly normally distributed for Gas and PHEV's and look to average around 5 (middle rating)

```{r}
#Plot Fuel Efficiency Rating Distribution
ggplot(model, aes(x=fe_rating)) + 
  geom_histogram(binwidth = 1) +
  ggtitle("Distribution of Fuel Efficiency Rating")+
  theme_minimal()

```
<br/>

### Check for linear relationship between predictor and response variable
...
<br/> In this case, we would consider dropping the transmission type variable.
...
* Ideally we would drop the redundant variables

<br/> In this case, combined, city and highway are collinear variables.  Ideally we would drop 2 redundant variable and keep only one.  
...
* Ideally, we would only keep variables where p < .05
...
### Profile of a fuel efficient non EV vehicle
...
The profile of a gas or plug-in vehicle that predicts the efficiency rating: 

* a decreasing number of engine cylinders
* an increasing combined mpg
* does not have a part-time 4 wheel drive train
* a decreasing weight of C02 emissions
* not a cab chassis truck
* is a 2WD drive minivan
...
* Fuel and vehicle information reported by car manufacturers and summarized by the EPA is complex.
* As a consumer, it is difficult to evaluate efficiency and cost due to the shear number of vehicle types and variables.
* Further analysis and tools are needed to be able to identify the vehicle variables that matter and compare them.
